import os, json, uuid, sqlite3, secrets, base64, time, hashlib, qrcode, io
...
# ─── HTML Templates ─────────────────────────────────────
LOGIN_HTML = """<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>XBZ PRO v1.5.9</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:#0a0a0f;min-height:100vh;display:flex;align-items:center;justify-content:center;overflow:hidden}body::before{content:'';position:fixed;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(ellipse at 30% 20%,rgba(120,50,250,0.15),transparent 50%),radial-gradient(ellipse at 70% 80%,rgba(250,50,120,0.1),transparent 50%);animation:bg 8s ease-in-out infinite alternate}@keyframes bg{0%{transform:rotate(0deg)}100%{transform:rotate(3deg)}}.card{position:relative;z-index:1;background:rgba(255,255,255,0.03);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:48px 40px;width:420px;text-align:center}.logo{font-size:3.5em;margin-bottom:8px}.card h1{color:#fff;font-size:2em;margin-bottom:2px}.ver{color:#7832fa;font-size:0.8em;font-weight:600;margin-bottom:4px}.card .sub{color:#555;font-size:0.9em;margin-bottom:32px}.field input{width:100%;padding:14px 16px;border:1px solid rgba(255,255,255,0.1);border-radius:14px;background:rgba(255,255,255,0.04);color:#fff;font-size:0.95em;transition:0.3s;font-family:inherit;margin-bottom:14px}.field input:focus{border-color:rgba(120,50,250,0.5);outline:none;box-shadow:0 0 20px rgba(120,50,250,0.1)}.field input::placeholder{color:#444}.btn{width:100%;padding:15px;background:linear-gradient(135deg,#7832fa,#fa3278);border:none;border-radius:14px;color:#fff;font-size:1.05em;font-weight:700;cursor:pointer;transition:0.3s;font-family:inherit}.btn:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(120,50,250,0.3)}.err{color:#fa3278;margin-bottom:16px;font-size:0.85em}</style></head><body><div class="card"><div class="logo">⚡</div><h1>XBZ PRO</h1><div class="ver">v1.5.9</div><div class="sub">پنل مدیریت VPN</div>{% if error %}<div class="err">{{error}}</div>{% endif %}<form method="POST"><div class="field"><input name="username" placeholder="نام کاربری" required></div><div class="field"><input name="password" type="password" placeholder="رمز عبور" required></div><button type="submit" class="btn">ورود به پنل</button></form></div></body></html>"""

SETTINGS_HTML = """<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Settings</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:#0a0a0f;color:#e0e0e0;min-height:100vh}.nav{background:rgba(255,255,255,0.02);padding:16px 32px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.06)}.nav h2{color:#7832fa;font-size:1.3em}.nav a{color:#555;text-decoration:none;margin-right:16px;font-size:0.9em;transition:0.2s}.nav a:hover{color:#fa3278}.ct{max-width:700px;margin:40px auto;padding:0 24px}.sec{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:20px;padding:32px}.sec h3{color:#7832fa;margin-bottom:24px}.fg{margin-bottom:16px}.fg label{display:block;color:#666;font-size:0.82em;margin-bottom:6px}.fg input{width:100%;padding:12px 16px;border:1px solid rgba(255,255,255,0.1);border-radius:12px;background:rgba(255,255,255,0.04);color:#fff;font-size:0.95em;font-family:inherit}.fg input:focus{border-color:rgba(120,50,250,0.5);outline:none}.btn{padding:12px 32px;background:linear-gradient(135deg,#7832fa,#fa3278);border:none;border-radius:12px;color:#fff;font-weight:700;cursor:pointer;font-family:inherit}.info{background:rgba(120,50,250,0.08);border:1px solid rgba(120,50,250,0.2);border-radius:12px;padding:16px;margin-top:20px;font-size:0.85em;color:#aaa;line-height:2}.info code{color:#7832fa;background:rgba(120,50,250,0.1);padding:2px 8px;border-radius:6px;font-size:0.9em}</style></head><body><div class="nav"><h2>⚙️ Settings <span style="color:#444;font-size:0.7em">v{{version}}</span></h2><div><a href="/">داشبورد</a><a href="/logout">خروج</a></div></div><div class="ct"><div class="sec"><h3>تنظیمات پنل</h3><form method="POST"><div class="fg"><label>نام پنل</label><input name="panel_title" value="{{title}}"></div><div class="fg"><label>دامنه سرور</label><input name="server_domain" value="{{domain}}"></div><div class="fg"><label>پورت سرور</label><input name="server_port" value="{{port}}"></div><div class="fg"><label>مسیر اشتراک</label><input name="sub_path" value="{{sub_path}}"></div><button type="submit" class="btn">💾 ذخیره</button></form><div class="info"><strong>📌 لینک اشتراک:</strong><br><code>{{domain}}:{{port}}{{sub_path}}/{UUID}</code><br><strong>📌 صفحه اشتراک:</strong><br><code>{{domain}}:{{port}}/share/{UUID}</code><br><strong>📌 اطلاعات ساب:</strong><br><code>{{domain}}:{{port}}{{sub_path}}/{UUID}/info</code><br><strong>📌 خروجی JSON:</strong><br><code>{{domain}}:{{port}}/api/export</code><br><strong>📌 سازگار با:</strong> v2rayNG, Hiddify, Nekobox, V2Box, Streisand, Sing-box</div></div></div></body></html>"""

SHARE_HTML = """<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{{email}} - {{panel_name}}</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:#0a0a0f;color:#e0e0e0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}body::before{content:'';position:fixed;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(ellipse at 30% 20%,rgba(120,50,250,0.12),transparent 50%),radial-gradient(ellipse at 70% 80%,rgba(250,50,120,0.08),transparent 50%)}.card{position:relative;z-index:1;background:rgba(255,255,255,0.03);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.08);border-radius:24px;padding:36px;width:480px;max-width:100%}.logo{font-size:2.5em;margin-bottom:6px}.card h1{color:#fff;font-size:1.4em;margin-bottom:2px}.card .sub{color:#555;font-size:0.85em;margin-bottom:20px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px;text-align:right}.item{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.06);border-radius:12px;padding:12px}.item .lbl{color:#555;font-size:0.72em;margin-bottom:3px}.item .val{color:#fff;font-size:0.9em;font-weight:600}.qr-box{text-align:center;margin:16px 0}.qr-box img{border-radius:12px;border:2px solid rgba(255,255,255,0.1)}.link-box{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:12px;word-break:break-all;font-family:monospace;font-size:0.72em;color:#4caf50;margin-bottom:14px;text-align:left;direction:ltr;max-height:100px;overflow-y:auto}.btn{width:100%;padding:13px;background:linear-gradient(135deg,#7832fa,#fa3278);border:none;border-radius:14px;color:#fff;font-size:0.95em;font-weight:700;cursor:pointer;transition:0.3s;font-family:inherit;margin-bottom:8px}.btn:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(120,50,250,0.3)}.btn2{width:100%;padding:11px;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:12px;color:#888;font-size:0.88em;cursor:pointer;margin-bottom:8px;font-family:inherit;transition:0.2s}.btn2:hover{background:rgba(255,255,255,0.1);color:#fff}.badge{display:inline-block;padding:3px 10px;border-radius:8px;font-size:0.72em;font-weight:600}.bg{background:rgba(76,175,80,0.15);color:#4caf50}.br{background:rgba(244,67,54,0.15);color:#f44336}.toast{position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:#7832fa;color:#fff;padding:12px 24px;border-radius:12px;font-size:0.9em;display:none;z-index:100}</style></head><body><div class="card"><div class="logo">⚡</div><h1>{{panel_name}}</h1><div class="sub">{{email}}</div><div class="grid"><div class="item"><div class="lbl">پروتکل</div><div class="val">{{protocol}}</div></div><div class="item"><div class="lbl">وضعیت</div><div class="val">{% if enable %}<span class="badge bg">فعال</span>{% else %}<span class="badge br">غیرفعال</span>{% endif %}</div></div><div class="item"><div class="lbl">دامنه</div><div class="val" style="font-size:0.78em">{{domain}}</div></div><div class="item"><div class="lbl">پورت</div><div class="val">{{port}}</div></div><div class="item"><div class="lbl">ترافیک مصرفی</div><div class="val">{{traffic_str}}</div></div><div class="item"><div class="lbl">حجم کل</div><div class="val">{{total_str}}</div></div><div class="item"><div class="lbl">تاریخ انقضا</div><div class="val">{{exp_str}}</div></div><div class="item"><div class="lbl">UUID</div><div class="val" style="font-size:0.68em;font-family:monospace">{{uuid[:16]}}...</div></div></div><div class="qr-box"><img src="data:image/png;base64,{{qr_data}}" width="180"></div><div class="link-box" id="link">{{link}}</div><button class="btn" onclick="copyL()">📋 کپی لینک VPN</button><button class="btn2" onclick="copyS()">🔗 کپی لینک اشتراک</button><button class="btn2" onclick="copyI()">📊 کپی لینک اطلاعات</button></div><div class="toast" id="toast">✅ کپی شد!</div><script>function copyL(){navigator.clipboard.writeText(document.getElementById('link').innerText).then(()=>showT())}function copyS(){navigator.clipboard.writeText('{{sub_url}}').then(()=>showT())}function copyI(){navigator.clipboard.writeText('{{info_url}}').then(()=>showT())}function showT(){var t=document.getElementById('toast');t.style.display='block';setTimeout(()=>t.style.display='none',2000)}</script></body></html>"""

DASH_HTML = """<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{{panel_name}} v{{version}}</title><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Inter',sans-serif;background:#0a0a0f;color:#e0e0e0;min-height:100vh}.nav{background:rgba(255,255,255,0.02);padding:14px 32px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(255,255,255,0.06)}.nav h2{color:#7832fa;font-size:1.3em}.nav a{color:#555;text-decoration:none;margin-right:16px;font-size:0.88em;transition:0.2s}.nav a:hover{color:#fa3278}.ct{max-width:1200px;margin:24px auto;padding:0 24px}.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px;margin-bottom:24px}.stat{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:18px;text-align:center}.stat .num{font-size:1.7em;font-weight:800;background:linear-gradient(135deg,#7832fa,#fa3278);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.stat .label{color:#555;font-size:0.78em;margin-top:4px}.sec{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:20px;padding:24px;margin-bottom:20px}.sec h3{color:#7832fa;margin-bottom:16px;font-size:1.05em}table{width:100%;border-collapse:collapse}th,td{padding:9px 10px;text-align:right;border-bottom:1px solid rgba(255,255,255,0.04);font-size:0.8em}th{color:#444;font-weight:600;font-size:0.72em;text-transform:uppercase;letter-spacing:0.5px}.badge{display:inline-block;padding:3px 10px;border-radius:8px;font-size:0.72em;font-weight:600}.bg{background:rgba(76,175,80,0.15);color:#4caf50}.br{background:rgba(244,67,54,0.15);color:#f44336}.bb{background:rgba(33,150,243,0.15);color:#2196f3}.bp{background:rgba(120,50,250,0.15);color:#7832fa}.btn{padding:5px 12px;border:none;border-radius:8px;cursor:pointer;font-size:0.75em;margin:1px;transition:0.2s;font-family:inherit}.ba{background:linear-gradient(135deg,#7832fa,#fa3278);color:#fff}.ba:hover{opacity:0.85}.bd{background:rgba(244,67,54,0.15);color:#f44336}.bd:hover{background:rgba(244,67,54,0.3)}.bl{background:rgba(33,150,243,0.15);color:#2196f3}.bl:hover{background:rgba(33,150,243,0.3)}.bt{background:rgba(255,193,7,0.15);color:#ffc107}.bsh{background:rgba(120,50,250,0.12);color:#7832fa}.bsh:hover{background:rgba(120,50,250,0.25)}.binfo{background:rgba(0,188,212,0.12);color:#00bcd4}.binfo:hover{background:rgba(0,188,212,0.25)}input,select{padding:7px 11px;border:1px solid rgba(255,255,255,0.1);border-radius:10px;background:rgba(255,255,255,0.04);color:#fff;font-size:0.82em;margin:3px;font-family:inherit}input:focus,select:focus{border-color:rgba(120,50,250,0.4);outline:none}select option{background:#1a1a2e;color:#fff}.fr{display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin-bottom:10px}.cb{background:rgba(76,175,80,0.05);border:1px solid rgba(76,175,80,0.2);border-radius:12px;padding:14px;margin-top:10px;word-break:break-all;font-family:monospace;font-size:0.78em;color:#4caf50;display:none}.uuid-text{font-family:monospace;font-size:0.7em;color:#666}.traffic{font-size:0.73em;color:#777}.mo{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:100;align-items:center;justify-content:center}.mo.show{display:flex}.md{background:#141420;border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:32px;width:520px;max-height:85vh;overflow-y:auto}.md h3{color:#7832fa;margin-bottom:20px}.md .cl{float:left;cursor:pointer;color:#555;font-size:1.2em;transition:0.2s}.md .cl:hover{color:#f44336}.toast{position:fixed;bottom:30px;left:50%;transform:translateX(-50%);background:#7832fa;color:#fff;padding:12px 24px;border-radius:12px;font-size:0.9em;display:none;z-index:200}.info-panel{background:rgba(0,188,212,0.05);border:1px solid rgba(0,188,212,0.15);border-radius:12px;padding:16px;margin-top:10px;display:none;font-size:0.82em;line-height:1.8}.info-panel .iplbl{color:#555}.info-panel .ipval{color:#00bcd4;font-weight:600}</style></head><body><div class="nav"><h2>⚡ {{panel_name}} <span style="color:#444;font-size:0.6em">v{{version}}</span></h2><div><a href="/settings">⚙️ تنظیمات</a><a href="/api/export" target="_blank">📦 خروجی</a><a href="/logout">خروج</a></div></div><div class="ct"><div class="stats"><div class="stat"><div class="num">{{total_inbounds}}</div><div class="label">اینباند</div></div><div class="stat"><div class="num">{{total_users}}</div><div class="label">کل کاربران</div></div><div class="stat"><div class="num">{{active_users}}</div><div class="label">فعال</div></div><div class="stat"><div class="num">{{expired_users}}</div><div class="label">منقضی</div></div><div class="stat"><div class="num">{{'%0.1f'|format(total_down/1073741824)}} GB</div><div class="label">دانلود کل</div></div></div><div class="sec"><h3>📡 اینباندها</h3><button class="btn ba" onclick="showM('inM')" style="margin-bottom:14px">➕ اینباند جدید</button><table><tr><th>#</th><th>نام</th><th>پروتکل</th><th>پورت</th><th>Domain</th><th>SNI</th><th>شبکه</th><th>وضعیت</th><th>عملیات</th></tr>{% for i in inbounds %}<tr><td>{{i.id}}</td><td style="font-weight:600">{{i.name}}</td><td><span class="badge bp">{{i.protocol|upper}}</span></td><td>{{i.port}}</td><td>{{i.domain}}</td><td>{{i.sni}}</td><td>{{i.network}}</td><td>{% if i.enable %}<span class="badge bg">فعال</span>{% else %}<span class="badge br">غیرفعال</span>{% endif %}</td><td><button class="btn bt" onclick="togIn({{i.id}})">🔄</button><button class="btn bd" onclick="delIn({{i.id}})">🗑️</button></td></tr>{% endfor %}</table></div><div class="sec"><h3>👤 کاربران</h3><div class="fr" style="margin-bottom:14px"><button class="btn ba" onclick="showM('usM')">➕ کاربر جدید</button><button class="btn ba" onclick="showM('batchM')" style="background:linear-gradient(135deg,#00bcd4,#7832fa)">📦 ساخت گروهی</button></div><table><tr><th>#</th><th>نام</th><th>UUID</th><th>اینباند</th><th>ترافیک</th><th>انقضا</th><th>وضعیت</th><th>عملیات</th></tr>{% for u in users %}<tr><td>{{u.id}}</td><td style="font-weight:600">{{u.email}}</td><td class="uuid-text">{{u.uuid[:16]}}...</td><td>{{u.inb_name or '-'}} <span class="badge bp" style="font-size:0.6em">{{u.inb_proto or ''}}</span></td><td class="traffic">{{'%0.2f'|format(u.up/1073741824)}}↑ / {{'%0.2f'|format(u.down/1073741824)}}↓ GB</td><td class="traffic">{{u.expiry//86400}} روز</td><td>{% if u.enable %}<span class="badge bg">فعال</span>{% else %}<span class="badge br">غیرفعال</span>{% endif %}</td><td><button class="btn bl" onclick="getL({{u.id}})">🔗 لینک</button><button class="btn bsh" onclick="shareP('{{u.uuid}}')">👁️ صفحه</button><button class="btn binfo" onclick="showInfo({{u.id}})">📊 اطلاعات</button><button class="btn bt" onclick="togU({{u.id}})">🔄</button><button class="btn bd" onclick="delU({{u.id}})">🗑️</button></td></tr>{% endfor %}</table><div id="lbox" class="cb"></div><div id="infoPanel" class="info-panel"></div></div></div><div class="mo" id="inM"><div class="md"><span class="cl" onclick="hideM('inM')">✕</span><h3>➕ اینباند جدید</h3><div class="fr"><input id="i-name" placeholder="نام" value="VLESS-NL" style="width:140px"><select id="i-proto"><option value="vless">VLESS</option><option value="vmess">VMess</option><option value="trojan">Trojan</option></select><input id="i-port" placeholder="پورت" value="443" type="number" style="width:65px"></div><div class="fr"><input id="i-domain" placeholder="Domain" style="width:230px"><input id="i-sni" placeholder="SNI" style="width:180px"></div><div class="fr"><input id="i-host" placeholder="Host" style="width:180px"><input id="i-path" placeholder="Path" value="/ws" style="width:90px"></div><div class="fr"><select id="i-net"><option value="ws">WebSocket</option><option value="grpc">gRPC</option><option value="tcp">TCP</option></select><select id="i-sec"><option value="none">None</option><option value="tls">TLS</option></select><button class="btn ba" onclick="addIn()">ذخیره</button></div></div></div><div class="mo" id="usM"><div class="md"><span class="cl" onclick="hideM('usM')">✕</span><h3>➕ کاربر جدید</h3><div class="fr"><input id="u-email" placeholder="نام کاربری" style="width:200px"></div><div class="fr"><select id="u-inbound" style="width:100%">{% for i in inbounds %}<option value="{{i.id}}">{{i.name}} ({{i.protocol}}:{{i.port}})</option>{% endfor %}</select></div><div class="fr"><input id="u-exp" placeholder="روز" type="number" value="30" style="width:70px"><input id="u-total" placeholder="GB" type="number" value="0" style="width:80px"><input id="u-limitip" placeholder="محدودیت IP" type="number" value="0" style="width:80px"></div><div class="fr"><input id="u-tgid" placeholder="Telegram ID" style="width:150px"><input id="u-comment" placeholder="توضیحات" style="width:170px"></div><div class="fr"><button class="btn ba" onclick="addU()">ذخیره</button></div></div></div><div class="mo" id="batchM"><div class="md"><span class="cl" onclick="hideM('batchM')">✕</span><h3>📦 ساخت گروهی کاربر</h3><div class="fr"><input id="b-count" placeholder="تعداد" type="number" value="10" style="width:80px"><input id="b-prefix" placeholder="پیشوند نام" value="user" style="width:120px"></div><div class="fr"><select id="b-inbound" style="width:100%">{% for i in inbounds %}<option value="{{i.id}}">{{i.name}} ({{i.protocol}}:{{i.port}})</option>{% endfor %}</select></div><div class="fr"><input id="b-exp" placeholder="روز" type="number" value="30" style="width:70px"><input id="b-total" placeholder="GB" type="number" value="0" style="width:80px"></div><div class="fr"><button class="btn ba" onclick="batchAdd()">📦 ساختن</button></div><div id="batchResult" style="margin-top:12px;font-size:0.82em;color:#4caf50"></div></div></div><div class="toast" id="toast">✅ کپی شد!</div><script>function showM(id){document.getElementById(id).classList.add('show')}function hideM(id){document.getElementById(id).classList.remove('show')}function g(id){return document.getElementById(id).value}function toast(){var t=document.getElementById('toast');t.style.display='block';setTimeout(()=>t.style.display='none',2000)}function addIn(){fetch("/api/inbound/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:g("i-name"),protocol:g("i-proto"),port:g("i-port"),domain:g("i-domain"),sni:g("i-sni"),host:g("i-host"),path:g("i-path"),network:g("i-net"),security:g("i-sec")})}).then(r=>r.json()).then(d=>{if(d.success)location.reload()})}function delIn(id){if(confirm("حذف شود؟"))fetch("/api/inbound/delete/"+id,{method:"POST"}).then(()=>location.reload())}function togIn(id){fetch("/api/inbound/toggle/"+id,{method:"POST"}).then(()=>location.reload())}function addU(){fetch("/api/user/add",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:g("u-email"),inbound_id:g("u-inbound"),expiry:g("u-exp"),total:g("u-total"),limit_ip:g("u-limitip"),tg_id:g("u-tgid"),comment:g("u-comment")})}).then(r=>r.json()).then(d=>{if(d.success)location.reload()})}function delU(id){if(confirm("حذف شود؟"))fetch("/api/user/delete/"+id,{method:"POST"}).then(()=>location.reload())}function togU(id){fetch("/api/user/toggle/"+id,{method:"POST"}).then(()=>location.reload())}function getL(id){fetch("/api/link/"+id).then(r=>r.json()).then(d=>{var b=document.getElementById("lbox");b.style.display="block";b.innerHTML="<strong>🔗 لینک VPN:</strong><br>"+d.link+"<br><br><strong>📋 لینک اشتراک:</strong><br>"+window.location.origin+"/sub/"+d.uuid+"<br><br><strong>📊 اطلاعات:</strong><br>"+window.location.origin+"/sub/"+d.uuid+"/info";navigator.clipboard.writeText(d.link||"").then(()=>toast())})}function shareP(uuid){window.open("/share/"+uuid,"_blank")}function showInfo(id){fetch("/api/user/info/"+id).then(r=>r.json()).then(d=>{var p=document.getElementById("infoPanel");p.style.display="block";p.innerHTML="<strong>📊 اطلاعات کاربر:</strong><br><span class='iplbl'>نام:</span> <span class='ipval'>"+d.email+"</span><br><span class='iplbl'>UUID:</span> <span class='ipval'>"+d.uuid+"</span><br><span class='iplbl'>پروتکل:</span> <span class='ipval'>"+d.protocol+"</span><br><span class='iplbl'>دامنه:</span> <span class='ipval'>"+d.domain+"</span><br><span class='iplbl'>آپلود:</span> <span class='ipval'>"+d.traffic_up+"</span><br><span class='iplbl'>دانلود:</span> <span class='ipval'>"+d.traffic_down+"</span><br><span class='iplbl'>حجم کل:</span> <span class='ipval'>"+d.traffic_total+"</span><br><span class='iplbl'>انقضا:</span> <span class='ipval'>"+d.expiry_date+"</span><br><span class='iplbl'>آخرین ورود:</span> <span class='ipval'>"+(d.last_login||'هیچوقت')+"</span><br><span class='iplbl'>لینک ساب:</span> <span class='ipval'>"+window.location.origin+"/sub/"+d.uuid+"</span>";window.scrollTo({top:p.offsetTop-100,behavior:'smooth'})})}function batchAdd(){fetch("/api/user/batch",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({count:g("b-count"),prefix:g("b-prefix"),inbound_id:g("b-inbound"),expiry:g("b-exp"),total:g("b-total")})}).then(r=>r.json()).then(d=>{if(d.success){var r=document.getElementById("batchResult");r.innerHTML="✅ "+d.count+" کاربر ساخته شد!<br><br>";d.users.forEach(u=>{r.innerHTML+="<code>"+u.email+"</code> → <code style='color:#7832fa'>"+u.uuid+"</code><br>"});r.innerHTML+="<br><button class='btn ba' onclick='location.reload()'>بازخوانی</button>"}})}</script></body></html>"""
